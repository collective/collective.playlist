...
class PlaylistIntegrationTest(unittest.TestCase):
    ...
    def test_BaseView(self):
        ...
        val = "https://fonts.googleapis.com/css?family=Ubuntu"
        api.portal.set_registry_record("collective.playlist.stylesheets", [val])

        view = self.portal.playlist.restrictedTraverse("@@playlistfooterview")
        tracks = [
            {
                "mp3": "http://nohost/plone/playlist/track-1/@@download/audiofile/track-1.mp3",  # noqa: 128
                "title": "Track 1",
            },
            {
                "mp3": "http://nohost/plone/playlist/track-2/@@download/audiofile/track-2.mp3",  # noqa: 128
                "title": "Track 2",
            },
            {
                "mp3": "http://nohost/plone/playlist/track-3/@@download/audiofile/track-3.mp3",  # noqa: 128
                "title": "Track 3",
            },
        ]
        self.assertEqual(view.getTracks(), tracks)
        self.assertEqual(view.copyright(), "")
        str = '<link rel="stylesheet" type="text/css" href="{0}" />'.format(val)
